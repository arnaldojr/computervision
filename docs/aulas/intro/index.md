# Introdução

Faça o download do PDF de Introdução:

- **Arquivo PDF:** [Introdução (slides.pdf)](slides.pdf)

!!! tip "Dica"
    Abra o PDF em uma aba e acompanhe junto com os exemplos abaixo. Você vai reconhecer os conceitos quando eles aparecerem nos laboratórios.

---

## Aplicações em Visão Computacional (vídeos)

A ideia destes vídeos é mostrar **onde a visão computacional aparece no mundo real** e o tipo de problema que vamos atacar na disciplina.

<div class="grid cards" markdown>

-   **Aplicação 1**

    [Assistir no YouTube](https://www.youtube.com/watch?v=CoRJG384yOg){ .md-button }

-   **Aplicação 2**

    [Assistir no YouTube](https://www.youtube.com/watch?v=t6XjeGr_-Pk){ .md-button }

-   **Aplicação 3**

    [Assistir no YouTube](https://www.youtube.com/watch?v=0iHXExrrJpw){ .md-button }

</div>

??? info "Ver vídeos embutidos (player)"
    <iframe width="100%" height="420"
        src="https://www.youtube.com/embed/CoRJG384yOg"
        title="Aplicação 1"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

    <iframe width="100%" height="420"
        src="https://www.youtube.com/embed/t6XjeGr_-Pk"
        title="Aplicação 2"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

    <iframe width="100%" height="420"
        src="https://www.youtube.com/embed/0iHXExrrJpw"
        title="Aplicação 3"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

---

## Projetos passados (inspiração)

A seguir estão playlists/vídeos de projetos de turmas anteriores. Use como referência para entender **o nível esperado** de protótipo e apresentação.

<div class="grid cards" markdown>

-   **Projeto 1**

    [Assistir](https://www.youtube.com/watch?v=znwVRa8RSHM&list=PLVU3UhXa4-X_yYRMpRoFiTtvv13FEdHaw&index=3){ .md-button }

-   **Projeto 2**

    [Assistir](https://www.youtube.com/watch?v=y6fS__BI33s&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra){ .md-button }

-   **Projeto 3**

    [Assistir](https://www.youtube.com/watch?v=m_PxgEP4mB8&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra&index=2){ .md-button }

-   **Projeto 4**

    [Assistir](https://www.youtube.com/watch?v=a_K95vKyil4&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra&index=3){ .md-button }

</div>

??? info "Ver vídeos embutidos (player)"
    <iframe width="100%" height="420"
        src="https://www.youtube.com/watch?v=znwVRa8RSHM&list=PLVU3UhXa4-X_yYRMpRoFiTtvv13FEdHaw&index=3"
        title="Projeto 1"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

    <iframe width="100%" height="420"
        src="https://www.youtube.com/watch?v=y6fS__BI33s&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra"
        title="Projeto 2"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

    <iframe width="100%" height="420"
        src="https://www.youtube.com/watch?v=m_PxgEP4mB8&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra&index=2"
        title="Projeto 3"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

    <iframe width="100%" height="420"
        src="https://www.youtube.com/watch?v=a_K95vKyil4&list=PLVU3UhXa4-X-xSRHE_0zGpH64yMK-_5Ra&index=3"
        title="Projeto 4"
        frameborder="0"
        allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
        allowfullscreen></iframe>

